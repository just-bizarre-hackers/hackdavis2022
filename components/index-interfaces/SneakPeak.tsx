    ...
    return (
        <div className="w-full relative">
            <section className={horizentalPaddingGrowth('w-full py-16')}>
                <h3 className="text-center text-themeNavy font-bold text-2xl mb-12">Have a Sneak Peack at Our Courses!</h3>
                <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-12 px-8 md:px-12 lg:px-20 xl:px-36 2xl:px-60">
                    {Categories.map((categories, index) => (
                                      <article
                                      onClick={() => onCardClick(1, categories.courses[0].title, categories.courses[0].desc)}
                                    ...
                                    >
                                      <div className="">
                                        ...
                                        <div className="w-full sticky text-sm bottom-0 bg-themeBlue hover:bg-themeNavy hover:bg-opacity-80 text-white text-center h-16 transition-all hover:h-full cursor-pointer flex flex-col justify-between pb-4 hover hover:bottom-[58px] transform hover:translate-y-[58px]">
                                          <h3 className="font-medium py-4 px-4 text-lg">{categories.title.slice(0, 21)}</h3>
